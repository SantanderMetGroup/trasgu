...
RESULTS_DIR = ANALYSIS_DIR.parent / "sample_size_scaling" / "results"
...
START = re.compile(
    rf"^{STAMP} .* Parallel fitting chunk (?P<chunk>\d+) .* using \d+ workers$"
)
...
def plot(rows: list[dict[str, object]], path: Path) -> None:
    ...
    ax.set(
        title="Summed Trasgu fitting time across all chunks",
        xlabel="Sample size",
        ylabel="Summed fitting time (hours)",
    )
    ax.set_xticks(x)
    fig.tight_layout()
    fig.savefig(path)
...
def main() -> None:
    ...
    csv_path = RESULTS_DIR / "total_execution_times.csv"
    if args.from_csv:
        rows = read_csv(csv_path)
    else:
        if args.root is None:
            parser.error("--root is required unless --from-csv is used")
        rows = collect(args.root, args.iteration)
        if not rows:
            raise SystemExit(
                "No complete fit_chunk logs found; use --from-csv to plot saved totals"
            )
        write_csv(rows, csv_path)
    plot(rows, RESULTS_DIR / "total_execution_time.png")
